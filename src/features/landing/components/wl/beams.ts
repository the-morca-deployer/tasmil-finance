...
function extendMaterial(BaseMaterial, cfg) {
  ...
  const baseVert = physical.vertexShader,
    baseFrag = physical.fragmentShader,
    baseUniforms = physical.uniforms;
  ...
  Object.entries(cfg.uniforms || {}).forEach(([k, u]) => {
...
export function mountBeams(canvas, opts) {
  opts = opts || {};
  const beamWidth = opts.beamWidth ?? 2,
    beamHeight = opts.beamHeight ?? 15,
    beamNumber = opts.beamNumber ?? 12,
    lightColor = opts.lightColor || "#ffffff",
    speed = opts.speed ?? 2,
    noiseIntensity = opts.noiseIntensity ?? 1.75,
  ...
  vec3 getNormal(vec3 pos) {
    ...
    material: { fog: true },
    uniforms: {
      diffuse: new THREE.Color(...hexRGB(opts.baseColor || "#0a1a22")),
      time: { value: 0 },
      roughness: 0.32,
      metalness: 0.45,
      uSpeed: { value: speed },
  ...
  const geometry = createStackedPlanes(beamNumber, beamWidth, beamHeight, 0, 100);
  const mesh = new THREE.Mesh(geometry, material);
  const group = new THREE.Group();
  group.rotation.z = THREE.MathUtils.degToRad(rotation);
  group.add(mesh);
  const dir = new THREE.DirectionalLight(lightColor, opts.lightIntensity ?? 2.4);
  ...
  window.__beams = { material, scene, camera, renderer, mesh, geometry };
