...
export function useMultiTrustlineCheck(
  ...
  const addTrustline = useCallback(
    async (contract: string, symbol: string): Promise<boolean> => {
      ...
      try {
        ...
        const tx = new TransactionBuilder(account, {
          ...
          .addOperation(
            Operation.changeTrust({ asset: new Asset(assetInfo.code, assetInfo.issuer) })
          )
          .setTimeout(180)
          .build();
...
async function checkAllTrustlines(
  address: string,
  tokens: TokenToCheck[]
): Promise<TokenToCheck[]> {
