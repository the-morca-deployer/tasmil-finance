      ...
      >
        <div className="space-y-1.5">
          ...
          {pool.protocol && (
            <DetailRow
              label="Protocol"
              value={<span className="capitalize">{pool.protocol}</span>}
            />
          )}
          {pool.reserveA != null && (
            <DetailRow label={`Reserve ${pool.tokenA}`} value={fmt(Number(pool.reserveA) / 1e7)} />
          )}
          {pool.reserveB != null && (
            <DetailRow label={`Reserve ${pool.tokenB}`} value={fmt(Number(pool.reserveB) / 1e7)} />
          )}
        </div>
      </ProtocolCard>
    );
