  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pt-8">
      ...
      <section className="relative w-full h-[400px] md:h-[500px] rounded-[32px] overflow-hidden group border border-white/25">
        ...
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to right, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.5) 25%, transparent 40%)' }}></div>
        {/* bottom fade */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>

        <div className="absolute inset-0 flex flex-col justify-center px-8 md:px-16 max-w-2xl space-y-6">
          <div className="flex items-center gap-2">
            <span className="text-brand-mid font-semibold tracking-wide text-sm uppercase">Tasmil Finance Quest</span>
          </div>
          ...
          <div className="pt-4">
            <Button variant="gradient" size="lg" className="rounded-full px-8" onClick={() => document.getElementById('campaigns')?.scrollIntoView({ behavior: 'smooth' })}>
              Start Questing
      ...
      <section id="campaigns" className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" size="icon" className="rounded-full" onClick={() => router.push('/quest/campaigns')}>
