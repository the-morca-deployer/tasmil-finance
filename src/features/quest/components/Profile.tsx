...
import { type ReferralTreeNode, unwrapEnvelope, type ReferralTree } from "@/features/quest/lib/season-types";
...
function Sidebar({ tab, setTab }: { tab: TabSlug; setTab: (t: TabSlug) => void }) {
  ...
  const items: { id: TabSlug; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "my-quest", label: "My Quests", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="18" height="18"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/></svg> },
...
// PtsCoin
const Pts = () => (
  <svg className="inline-block flex-none w-[18px] h-[18px]" style={{ verticalAlign: -4, marginLeft: 4 }} viewBox="0 0 24 24" fill="none" aria-hidden="true">
...
function OverviewTab() {
  ...
  const tierLabels: Record<string, string> = { bronze: "BR", silver: "SI", gold: "GO", diamond: "DI" };
  ...
  const [, toP, nextT] = progressMap[t] ?? [0, 15000, "Silver"];
  const progress = toP > 0 ? Math.min(1, Math.max(0, (points - (progressMap[t]?.[0] ?? 0)) / (toP - (progressMap[t]?.[0] ?? 0)))) : 1;
  const nextTierKey = tierOrder[tierCurrentIdx + 1] ?? "diamond";

  const tierColor = (key: string) => {
    if (key === "bronze") return "var(--bronze)";
    if (key === "silver") return "var(--silver)";
    if (key === "gold") return "var(--gold)";
    return "var(--diamond)";
  };
  const tierBorderColor = (key: string) => {
    if (key === "bronze") return "var(--bronze-line)";
    if (key === "silver") return "var(--silver-line)";
    if (key === "gold") return "var(--gold-line)";
    return "rgba(103,232,249,0.32)";
  };
  const tierBgColor = (key: string) => {
    if (key === "bronze") return "var(--bronze-soft)";
    if (key === "silver") return "var(--silver-soft)";
    if (key === "gold") return "var(--gold-soft)";
    return "var(--diamond-soft)";
  };
  ...
  return (
    <div>
      ...
      <div className="grid gap-[14px] items-start" style={{ gridTemplateColumns: "minmax(0,1fr) 320px" }}>
        <div className="flex flex-col gap-[14px] min-w-0">
          ...
          <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] overflow-hidden">
            <div className="px-[30px] pt-[28px] pb-[22px]">
              <div className="flex items-center justify-between gap-4 mb-[18px]">
                <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)] whitespace-nowrap">Current Points</div>
                <span
                  ...
                  style={{ color: tierColor(t), background: tierBgColor(t), borderColor: tierBorderColor(t) }}
                >
                    ...
                    return (
                      <div
                        ...
                        style={{
                          width: 44,
                          padding: "7px 3px",
                          background: isOn ? tierBgColor(key) : "none",
                          color: isOn ? tierColor(key) : "rgba(244,247,251,0.34)",
                          border: "none",
                        }}
                      >
                        <img
                          ...
                          src={mapTier[key] ?? mapTier.bronze}
                          alt=""
                        />
                        {tierLabels[key]}
                      </div>
            ...
            <div className="flex items-center gap-[18px] px-[26px] py-[22px] border-t border-[rgba(255,255,255,0.08)]">
              ...
              <div className="flex-1">
                <div className="relative h-3 rounded-[100px] border border-[rgba(255,255,255,0.08)] overflow-hidden" style={{ background: "var(--surface)" }}>
                  <div
                ...
                <div className="mt-[9px] text-[12px] text-[rgba(244,247,251,0.58)] text-center">
                  <b className="text-[var(--accent)] font-mono">{fmt(Math.max(0, toP - points))}</b>
                  <Pts />
                  {" "}to reach {nextT}
                </div>
              </div>

              <div className="flex flex-col items-center gap-[7px] flex-none w-16" style={{ opacity: 0.55 }}>
                <img
                  className="object-contain [filter:drop-shadow(0_5px_9px_rgba(0,0,0,0.45))]"
                  style={{ width: 42, height: 42 }}
                  src={mapTier[nextTierKey] ?? mapTier.silver}
                  alt=""
                />
                <div
                  className="text-[9px] font-bold tracking-[0.12em] uppercase"
                  style={{ color: tierColor(nextTierKey) }}
                >
                  {nextT}
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-[14px]">
            <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] px-[22px] py-[20px] flex flex-col">
              <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)]">Daily Streak</div>
        ...
        <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] flex flex-col min-w-0" style={{ padding: 22 }}>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="sm" block>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="13" height="13"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><path d="M16 6l-4-4-4 4"/><path d="M12 2v13"/></svg>
              Share Link
            </Button>
          </div>
          ...
          <div className="rounded-[14px] border border-[rgba(255,255,255,0.08)] mt-3" style={{ background: "rgba(32,32,36,0.30)", padding: "16px 18px" }}>
            <div className="flex items-center justify-between gap-[10px]">
              <span className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)]">Referral Rates</span>
...
function MyQuestsTab() {
  ...
  const items = useMemo(() => {
    const d = rawData as { success?: boolean; data?: { items?: ApiCampaign[] }; items?: ApiCampaign[] } | ApiCampaign[] | undefined;
    if (!d) return [];
        ...
        {subtabs.map((s) => (
          <button
            ...
            className={`relative inline-flex items-center gap-2 text-[14px] font-semibold bg-none border-none px-4 py-3 cursor-pointer transition-[color] duration-[250ms] ${
              subtab === s.id ? "text-[var(--accent)]" : "text-[rgba(244,247,251,0.58)] hover:text-[var(--text)]"
            }`}
            ...
            return (
              ...
              >
                ...
                <div className="relative h-[128px] overflow-hidden border-b border-[rgba(255,255,255,0.08)] bg-[#0c0e10]">
                  {cover ? (
                    <img alt="" src={cover} className="absolute inset-0 h-full w-full object-cover" />
                  ) : (
                    <div
                ...
                <div className="flex flex-col flex-1 px-5 py-[18px]">
                  <div className="text-[16px] font-bold tracking-[-0.02em] mb-[7px]">{ct.title as string}</div>
                  <div className="text-[13px] text-[rgba(244,247,251,0.58)] leading-[1.55] mb-4 flex-1">{(ct.description as string) ?? ""}</div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant={subtab === "claimable" ? "primary" : subtab === "pending" ? "accent" : "ghost"}
                      size="sm"
  ...
  return (
    <div>
      ...
      >
        ...
        />
        {/* name */}
        <span className="flex-1 min-w-0 font-mono text-[13px] font-semibold truncate">{node.name}</span>
        {/* badges */}
...
function ReferralsTab() {
  ...
  const howCards = [
    {
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="19" height="19"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13A4 4 0 0 1 16 11"/></svg>
      ),
  ...
  return (
    <div>
      ...
      <div className="grid gap-[18px] mb-[14px]" style={{ gridTemplateColumns: "1fr 280px", alignItems: "stretch" }}>
        {/* ref-hero */}
        <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] px-[30px] py-[28px]">
          ...
          <p className="text-[14px] text-[rgba(244,247,251,0.58)] leading-[1.6] mt-4 mb-[26px] max-w-[440px]">
            Earn from your referrals' quest points across three layers, forever. No expiry and no minimum.
          </p>

          {/* codebox */}
          <div className="border border-[rgba(255,255,255,0.14)] rounded-[14px] px-5 py-[18px]" style={{ background: "rgba(32,32,36,0.30)" }}>
            <div className="flex items-center justify-between gap-3 mb-[10px]">
              <span className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)]">Your Referral Code</span>
              <button
                ...
                onClick={() => {}}
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="13" height="13"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
                Set custom code
              </button>
            </div>
            ...
            <div className="flex gap-[10px]">
              <Button variant="primary" size="sm" onClick={() => { navigator.clipboard?.writeText(referralCode); toast.success("Copied!"); }}>
                <Copy size={13} /> Copy Code
              </Button>
        ...
        <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] px-[22px] py-[22px] flex flex-col">
          <div className="flex items-center justify-between mb-[18px]">
            ...
            <span className="text-[10px] font-bold tracking-[0.08em] uppercase text-[var(--accent)] bg-[var(--accent-soft)] border border-[rgba(103,232,249,0.32)] px-[10px] py-1 rounded-[100px]">Tier 1</span>
          </div>

          <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)]">Commission Amount</div>
          <div className="text-[32px] font-extrabold font-mono text-[var(--accent)] leading-none mt-1" style={{ letterSpacing: "-0.02em" }}>
            {fmt(totalEarned)}<Pts />
          </div>

          <div className="h-px bg-[rgba(255,255,255,0.08)] my-[18px]" />

          <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)]">Earn Rate (F1)</div>
          <div className="text-[24px] font-extrabold font-mono mt-1" style={{ letterSpacing: "-0.02em" }}>
            {Math.round(l1Rate / 100)}%
          </div>
          ...
          <div className="flex gap-[30px]">
            ...
            <div>
              <div className="text-[20px] font-bold font-mono text-[var(--green)]">{activeReferrals}</div>
              <div className="text-[10px] font-semibold tracking-[0.1em] uppercase text-[rgba(244,247,251,0.34)] mt-0.5">Active</div>
            </div>
          </div>
          ...
          <div className="mt-auto pt-[18px]">
            ...
            <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-[rgba(244,247,251,0.34)] mb-2">Quest PTS toward Tier 2</div>
            <div className="relative h-3 rounded-[100px] border border-[rgba(255,255,255,0.08)] overflow-hidden" style={{ background: "var(--surface)" }}>
              <div className="h-full rounded-[100px] [background:var(--grad)]" style={{ width: "46%" }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-[14px] mt-[14px]">
        {howCards.map((h) => (
          <div key={h.tier} className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] p-[22px]">
            <div className="flex items-start justify-between mb-[18px]">
              ...
              <span className="text-[28px] font-extrabold font-mono text-[var(--accent)]" style={{ letterSpacing: "-0.02em" }}>{h.pct}</span>
            </div>
            <div className="text-[9px] font-bold tracking-[0.14em] uppercase text-[rgba(244,247,251,0.34)] mb-1">{h.tier}</div>
      ...
      <div className="mt-[26px]">
          ...
          {(["list", "tree"] as const).map((v) => (
            <button
              ...
              className={`relative text-[11px] font-bold tracking-[0.14em] uppercase bg-none border-none px-[14px] py-[11px] cursor-pointer transition-[color] duration-200 ${
                view === v ? "text-[var(--accent)]" : "text-[rgba(244,247,251,0.58)] hover:text-[var(--text)]"
              }`}
              style={{ background: "none" }}
            >
        ...
        {view === "list" ? (
          refs.length === 0 ? (
            ...
            <div className="border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] overflow-hidden">
              ...
              <div
                ...
                style={{ gridTemplateColumns: "1.6fr 0.7fr 1fr 1fr", background: "rgba(0,0,0,0.25)" }}
              >
                <span>Username</span>
                <span>Layer</span>
                <span>Joined</span>
              ...
              {refs.map((r, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-[10px] min-w-0">
                    ...
                    <span className="font-mono text-[13px] font-semibold truncate">{r.username ?? "User"}</span>
                  </div>
                  {/* layerb */}
                  <span>
                    <span
                  ...
                  <span className="font-mono text-[13px] text-right tabular-nums text-[var(--accent)]">
                    {((r.questPoints ?? 0) + (r.ptsEarned ?? 0)).toLocaleString()}<Pts />
                  </span>
                </div>
              ))}
            </div>
          ...
          treeLoading ? (
            <div className="text-center py-12 text-[13px] text-[rgba(244,247,251,0.58)]">Loading tree…</div>
          ) : treeNodes.length === 0 ? (
            <div className="text-center py-12 text-[13px] text-[rgba(244,247,251,0.58)]">
...
function SocialTab() {
  ...
  const socials = [
    { platform: "X" as const, label: "X (Twitter)", icon: <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> },
    { platform: "Discord" as const, label: "Discord", icon: <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22"><path d="M20.317 4.369a19.79 19.79 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.211.375-.445.865-.608 1.249a18.27 18.27 0 0 0-5.487 0 12.6 12.6 0 0 0-.617-1.249.077.077 0 0 0-.079-.037A19.74 19.74 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.058a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.1 13.1 0 0 1-1.872-.892.077.077 0 0 1-.008-.128c.126-.094.252-.192.372-.291a.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.009c.12.099.246.198.373.292a.077.077 0 0 1-.006.127c-.598.349-1.22.645-1.873.892a.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.84 19.84 0 0 0 6.002-3.03.077.077 0 0 0 .032-.057c.5-5.177-.838-9.673-3.549-13.66a.06.06 0 0 0-.031-.029zM8.02 15.331c-1.182 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/></svg> },
    { platform: "Telegram" as const, label: "Telegram", icon: <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg> },
  ];
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 max-w-[620px]">
        {socials.map((s) => {
          ...
          return (
            <div key={s.platform} className="flex items-center gap-4 px-[22px] py-[18px] border border-[rgba(255,255,255,0.08)] rounded-[22px] [background:var(--card-grad)] transition-[border-color] duration-[250ms] hover:border-[rgba(103,232,249,0.32)]">
              <div className="w-[46px] h-[46px] rounded-[13px] flex-none grid place-items-center bg-[var(--surface)] border border-[rgba(255,255,255,0.14)]">{s.icon}</div>
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-bold tracking-[-0.02em]">{connected ? (acc?.username ?? s.platform) : s.label}</div>
                <div className={`text-[12.5px] mt-[2px] ${connected ? "text-[var(--green)]" : "text-[rgba(244,247,251,0.58)]"}`}>
...
function ProfileContent() {
  ...
  return (
    <div className="relative left-1/2 w-screen max-w-none -translate-x-1/2 grid min-h-[calc(100vh-64px)] grid-cols-[220px_1fr] items-stretch [margin-top:-40px] [margin-bottom:-120px] max-[720px]:grid-cols-1 max-[720px]:[margin-top:-40px] max-[720px]:[margin-bottom:-120px]">
      ...
      <div className="border-l border-[var(--line-2)] max-[720px]:border-l-0" style={{ padding: "0 clamp(24px, 5vw, 48px)" }}><div className="pt-8">
        <Rise delay={0.08}>
