  ...
  return (
    <div>
      ...
      <div className="relative">
        <div className="absolute top-[-330px] left-1/2 -translate-x-1/2 w-screen h-[1020px] pointer-events-none z-0 overflow-visible" aria-hidden="true" />
          ...
          <div className="grid [grid-template-columns:1.12fr_0.66fr_1.28fr]">
            ...
            <div className="px-[26px] py-[24px] flex flex-col justify-center items-center text-center">
              ...
              <div className="flex items-center gap-[13px] mb-[20px]">
                ...
                <div>
                  {/* bn-lab */}
                  <div className="text-[12.5px] font-bold tracking-[0.16em] uppercase text-quest-dim">This month's prize pool</div>
                  {/* bn-note */}
                  <div className="text-[14px] text-quest-muted mt-[5px]"><b className="text-quest-text font-bold">{rows.length}</b> players competing</div>
                </div>
              </div>
              {/* bn-figures */}
            ...
            <div className="px-[26px] py-[24px] flex flex-col justify-center items-center text-center border-l border-quest-line">
              ...
              <div className="flex gap-[10px] justify-center">
                <div className="flex flex-col items-center gap-[5px]">
                  <span className="font-mono text-[26px] font-bold text-quest-accent bg-[rgba(255,255,255,0.06)] border border-quest-accent-line rounded-[10px] py-[6px] px-[10px] min-w-[50px] text-center tracking-[-0.02em]">{String(cd.d).padStart(2, "0")}</span>
                  <span className="text-[9.5px] font-semibold tracking-[0.14em] uppercase text-quest-dim">Days</span>
                </div>
                <div className="flex flex-col items-center gap-[5px]">
                  <span className="font-mono text-[26px] font-bold text-quest-accent bg-[rgba(255,255,255,0.06)] border border-quest-accent-line rounded-[10px] py-[6px] px-[10px] min-w-[50px] text-center tracking-[-0.02em]">{String(cd.h).padStart(2, "0")}</span>
                  <span className="text-[9.5px] font-semibold tracking-[0.14em] uppercase text-quest-dim">Hrs</span>
                </div>
                <div className="flex flex-col items-center gap-[5px]">
                  <span className="font-mono text-[26px] font-bold text-quest-accent bg-[rgba(255,255,255,0.06)] border border-quest-accent-line rounded-[10px] py-[6px] px-[10px] min-w-[50px] text-center tracking-[-0.02em]">{String(cd.m).padStart(2, "0")}</span>
                  <span className="text-[9.5px] font-semibold tracking-[0.14em] uppercase text-quest-dim">Min</span>
                </div>
                <div className="flex flex-col items-center gap-[5px]">
                  <span className="font-mono text-[26px] font-bold text-quest-accent bg-[rgba(255,255,255,0.06)] border border-quest-accent-line rounded-[10px] py-[6px] px-[10px] min-w-[50px] text-center tracking-[-0.02em]">{String(cd.s).padStart(2, "0")}</span>
                  <span className="text-[9.5px] font-semibold tracking-[0.14em] uppercase text-quest-dim">Sec</span>
                </div>
              </div>
            </div>
            ...
            <div className="px-[26px] py-[24px] flex flex-col justify-center gap-[10px] border-l border-quest-line [background:linear-gradient(160deg,var(--color-quest-accent-soft),transparent_88%)]">
              ...
              <div className="flex items-baseline gap-[9px] flex-wrap mt-[12px]">
                <b className="text-[28px] font-extrabold tracking-[-0.03em] text-quest-accent leading-none whitespace-nowrap">{myResult ? `#${myResult.finalRank}` : "--"}</b>
        ...
        <div className="grid [grid-template-columns:8fr_4fr] gap-[22px] items-start max-[860px]:[grid-template-columns:1fr]">
          ...
          <aside className="flex flex-col gap-[18px] sticky top-[84px]">
            ...
            <div className="border border-quest-line rounded-quest-card [background:linear-gradient(160deg,rgba(32,32,36,0.45),rgba(16,16,18,0.45))] p-[22px]">
              ...
              {/* side-sub */}
              <div className="text-[12px] text-quest-dim mb-[16px]">USDC + Points, paid out at month end</div>
              {/* prize-card p1 */}
              ...
              {[4, 5, 6, 7, 8, 9, 10].map((r) => {
                ...
                return (
                  <div key={r} className="flex items-center gap-[10px] py-[9px] text-[13px]">
                    <span className="font-mono text-quest-muted font-semibold min-w-[32px]">{r}th</span>
                    <span className="flex-1 border-b border-quest-line -translate-y-[3px]" />
                    <span className="font-mono font-bold text-quest-accent">+{(ptsMap[r] ?? 0).toLocaleString()} <PtsCoin /></span>
                  </div>
                );
              })}
            </div>
