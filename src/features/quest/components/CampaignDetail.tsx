        ...
        <div className="px-4 pb-5 pl-[72px] space-y-4 animate-in slide-in-from-top-2 fade-in duration-300">
          ...
          <div className="flex flex-wrap gap-3 pt-1">
            ...
            {(() => {
              ...
              const rawStatus = (
                taskStatusData?.data as { status?: string }
              )?.status?.toUpperCase();
              const isVerified =
...
// Skeleton loading component — defined at module level to avoid nested component definition
const QuestSkeleton = () => (
        ...
        if (event.data?.status === "success" && event.data?.accountData) {
          try {
            ...
            linkAccountMutation.mutate(
              ...
              } as Parameters<typeof linkAccountMutation.mutate>[0],
              {
                onSuccess: () => {
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-bold flex items-center gap-2">
              <span className="w-1 h-6 bg-brand-mid rounded-full" />
              Quests
            </h3>
                      ...
                      return (
                        <QuestItem
                          ...
                          step={{
                            ...
                            type: mapTaskType(
                              (task as unknown as { type?: string }).type || task.taskType
                            ),
                            actionUrl:
                              (task as unknown as { metadata?: { urlAction?: string } }).metadata
                                ?.urlAction ||
                              task.urlAction ||
                              task.actionUrl ||
                              "#",
                            ...
                            points:
                              (task as unknown as { pointReward?: number }).pointReward ||
                              undefined,
                            checkId: (task as unknown as { metadata?: { checkId?: string } })
                              .metadata?.checkId,
                          }}
                        />
                      );
