...
export function Podium({
  rows,
  metric,
  usdcRewards = ["50", "20", "10"],
  ptsRewards = [5000, 3000, 2000],
}: PodiumProps) {
  ...
  return (
    <div className="relative">
      <div
        ...
        aria-hidden="true"
      />
      <section className="relative z-[1] grid items-end gap-[10px] max-w-[680px] mx-auto mt-[78px] mb-[64px] overflow-visible [grid-template-columns:1fr_1.18fr_1fr]">
        {ordered.map((r, i) => {
          if (!r) return <div key={i} className={`pod ${podClasses[i]}`} />;
          const place = placeIdx[i]!;
          const scoreStr = metric === "points" ? `${fmt(r.score)} pts` : `${r.score}-day streak`;
          ...
          return (
            <div key={r.rank} className="flex flex-col items-center min-w-0 overflow-visible">
              <div className="relative mb-[11px] overflow-visible">
                ...
                <span
                  className={cn(
                    "block w-[64px] h-[64px] rounded-full overflow-hidden",
                    is1st &&
                      "w-[92px] h-[92px] border-[3px] border-quest-gold [box-shadow:0_0_48px_rgba(251,197,74,0.5),inset_0_0_18px_rgba(0,0,0,0.18)]",
                    is2nd &&
                      "border-[3px] border-quest-silver [box-shadow:0_0_24px_rgba(201,212,224,0.28)]",
                    is3rd &&
                      "border-[3px] border-quest-bronze [box-shadow:0_0_24px_rgba(224,145,90,0.3)]"
                  )}
                >
                  <TasmilAvatar seed={r.address} size="full" />
                </span>
              </div>
              <div
                className={cn(
              ...
              <div className="flex items-center justify-center mt-[9px]">
                <span className="inline-flex items-center gap-[5px] font-mono text-[13px] font-bold py-[5px] px-[12px] rounded-quest-pill whitespace-nowrap text-quest-accent bg-quest-accent-soft border border-quest-accent-line">
                  ...
                  <svg
                    ...
                    aria-hidden="true"
                  >
                    <linearGradient id="ppGrad" x1="0.15" y1="0.1" x2="0.85" y2="0.9">
                      <stop stopColor="#A5F3FC" />
                      <stop offset="1" stopColor="#0EA5E9" />
                    </linearGradient>
                    <circle cx="12" cy="12" r="9" fill="url(#ppGrad)" />
              ...
              <div className="relative w-full mt-[20px] flex flex-col">
                ...
                <div
                  className={cn(
                    ...
                    !is1st &&
                      "[background:linear-gradient(180deg,#19191d,#0d0d10)] [box-shadow:inset_0_2px_0_rgba(255,255,255,0.13),inset_1px_0_0_rgba(255,255,255,0.06),inset_-2px_0_14px_-7px_rgba(0,0,0,0.6),inset_0_-18px_30px_-18px_rgba(0,0,0,0.7),0_34px_52px_-26px_#000]",
                    is1st &&
                      "h-[170px] [background:linear-gradient(180deg,#23232a,#0e0e12)] [box-shadow:inset_0_2px_0_rgba(255,255,255,0.18),inset_1px_0_0_rgba(255,255,255,0.12),inset_0_-18px_32px_-18px_rgba(0,0,0,0.7),0_0_88px_-24px_rgba(150,162,178,0.18),0_36px_56px_-24px_#000]",
                    is2nd && "h-[122px]",
                    is3rd && "h-[98px]"
                  )}
                >
                  <span
                    className={cn(
                      ...
                      is1st &&
                        "text-[92px] [background-image:linear-gradient(180deg,#FFF4CC_0%,#FBC54A_52%,#C8860B_100%)] [filter:drop-shadow(0_3px_4px_rgba(0,0,0,0.5))_drop-shadow(0_0_24px_rgba(251,197,74,0.55))]",
                      is2nd &&
                        "[background-image:linear-gradient(180deg,#FFFFFF_0%,#CBD6E2_52%,#8F9EAE_100%)]",
                      is3rd &&
                        "[background-image:linear-gradient(180deg,#FFDDBE_0%,#E0915A_52%,#AA5C2C_100%)]"
                    )}
                  >
                    {r.rank}
                  </span>
