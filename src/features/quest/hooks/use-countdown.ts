...
function compute(endAt: string | undefined): Countdown {
  if (!endAt) {
...
/**
 * Generic live countdown to `endAt` (ISO string). Recomputes every second.
 * Zero-padded; `ended` is true once the deadline passes or `endAt` is missing.
 */
export function useCountdown(endAt: string | undefined): Countdown {
  const [value, setValue] = useState<Countdown>(() => compute(endAt));
