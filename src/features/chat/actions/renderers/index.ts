...
for (const { toolName, entry } of BLEND_RENDERER_ENTRIES) toolRendererRegistry.register(toolName, entry);
for (const { toolName, entry } of AQUARIUS_RENDERER_ENTRIES) toolRendererRegistry.register(toolName, entry);
...
export { TASMIL_INFO_TOOLS, SUPERVISOR_AGENTS } from "./protocol-data";
