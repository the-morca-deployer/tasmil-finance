...
jest.mock("@/features/chat/config", () => ({
  getAgentConfig: () => ({ id: "supervisor", name: "Supervisor", icon: "/agents/supervisor-agent.png" }),
}));
...
test("Phase 2 first login shows welcome card with claim link", () => {
  render(
    <Greeting agentId="supervisor" phase="beta" isFirstLogin daysSinceLastStake={0} lastPoolEarnings={null} />,
  );
...
test("Phase 3 new user shows real-funds card", () => {
  render(
    <Greeting agentId="supervisor" phase="mainnet" isFirstLogin daysSinceLastStake={0} lastPoolEarnings={null} />,
  );
...
test("No phase card when conditions not met — renders default content", () => {
  render(
    <Greeting agentId="supervisor" phase="beta" isFirstLogin={false} daysSinceLastStake={2} lastPoolEarnings={null} />,
  );
