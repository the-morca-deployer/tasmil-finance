...
function ChatStateProvider({ children, initialThreadId = null }: ChatStateProviderProps) {
  ...
  const value = useMemo<ChatStateContextValue>(
    ...
    [threadId, hideToolCalls, chatHistoryOpen, handleSetChatHistoryOpen, assistantInfo]
  );
...
export function ChatProvider({ children, initialThreadId, apiKey }: ChatProviderProps) {
  return (
    <LangGraphProvider apiKey={apiKey}>
      <ChatStateProvider initialThreadId={initialThreadId}>{children}</ChatStateProvider>
    </LangGraphProvider>
  );
}
