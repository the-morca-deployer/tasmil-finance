...
import { useChatState, useLangGraph, usePendingMessage } from "@/features/chat-v2/providers";
import { historyService } from "@/features/chat-v2/services";
import type { ContentBlock, ToolCall, UniversalMessage } from "@/features/chat-v2/types";
...
export function useChatSession(options: UseChatSessionOptions) {
  ...
  useEffect(() => {
    if (!isNewChat && pendingMessage && isInitialized && !hasSentPendingMessage.current) {
  ...
  const sendMessage = useCallback(
    async (content: string, _attachments?: ContentBlock[]) => {
      try {
        setError(null);
