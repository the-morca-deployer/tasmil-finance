...
import {
  ...
  AssistantMessage,
  HumanMessage,
} from "@/features/chat-v2/components/messages";
        ...
        return (
          <div key={firstMessage.id || `assistant-${group.index}`}>
            ...
            {firstMessage.metadata?.["generativeUI"] ? (
              <div className="mt-2 ml-11">{renderGenerativeUI(firstMessage)}</div>
            ) : null}
            ...
            {group.followingToolCalls.length > 0 && (
              <div className="mt-2 ml-11 flex flex-col gap-2">
                {group.followingToolCalls.map((msg, idx) => {
                  ...
                  if (ui) {
                    return (
                      <Fragment key={msg.id || `following-tool-${group.index}-${idx}`}>
                        {ui}
                      </Fragment>
                    );
                  }
