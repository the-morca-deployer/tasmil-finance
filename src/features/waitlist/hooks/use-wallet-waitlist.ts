...
async function requestChallenge(walletAddress: string) {
  ...
  return unwrap(raw) as { challenge: string; nonce: string; expiresAt: string };
}
...
async function verifyReferralCode(code: string) {
  ...
  const raw = await res.json();
  return unwrap(raw) as { valid: boolean; inviterWalletAddress: string | null };
}
...
async function attachWaitlistContact(payload: { walletAddress: string; email: string }) {
  ...
  return unwrap(raw) as { success: boolean; alreadyHasEmail: boolean; email: string | null };
}
