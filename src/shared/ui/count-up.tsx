...
const formatWithAbbreviation = (num: number, decimals = 2): string => {
  ...
  if (absNum >= 1_000_000_000) {
    ...
    const significantDecimals = Math.max(decimals, Math.abs(Math.floor(Math.log10(absNum))) + 1);
  ...
  const trimmed = formatted.replace(/(\.\d*?)0+$/, "$1").replace(/\.$/, "") + suffix;
      ...
      <CountUpLib
        decimals={
          Math.abs(value) > 0 && Math.abs(value) < 0.01
            ? Math.max(decimals, Math.abs(Math.floor(Math.log10(Math.abs(value)))) + 1)
            : decimals
        }
        duration={duration}
        end={value}
