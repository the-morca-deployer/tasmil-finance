...
function ParallaxText({ children, baseVelocity = 100, ...props }: ParallaxProps) {
  ...
  const x = useTransform(baseX, (v: any) => `${wrap(-100 / repetitions, 0, v)}%`);

  const directionFactor = React.useRef<number>(1);
  useAnimationFrame((_t: any, delta: any) => {
...
export function VelocityScroll({
  ...
  return (
    <div
      className={cn(
        "relative w-full font-bold text-4xl tracking-[-0.02em] md:text-7xl md:leading-[5rem]",
        className
      )}
      {...props}
    >
