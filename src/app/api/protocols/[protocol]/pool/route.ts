...
import { getClient, isValidProtocol, jsonError, getNetwork, getExplorerUrl } from "../../_sdk";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ protocol: string }> }) {
  ...
  if (!SUPPORTS_POOL.has(protocol)) return jsonError(`${protocol} does not support single pool query`);
  ...
  try {
    ...
    if (protocol === "blend") {
      ...
      return Response.json({
        ...
        pool: {
          address: pool.address, name: pool.name, status: pool.status,
          backstopRate: pool.backstopRate,
          explorerUrl: getExplorerUrl(pool.address),
          reserves: pool.reserves.map((r) => ({
            assetAddress: r.assetAddress, symbol: r.symbol,
            ...
            liabilityFactor: r.liabilityFactor, decimals: r.decimals,
    ...
    if (protocol === "aquarius") {
      ...
      return Response.json({
        success: true, network, protocol,
        pool: {
          address: pool.address, poolType: pool.pool_type,
          tokens: pool.tokens, tokensStr: pool.tokens_str,
          fee: pool.fee, tvl: pool.total_value_locked,
          volume24h: pool.volume_24h, feeApy: pool.fee_apy,
    ...
    if (protocol === "defindex") {
      ...
      return Response.json({
        success: true, network, protocol,
        pool: {
          address: vault.address, name: vault.name,
          asset: vault.asset, assetAddress: vault.assetAddress,
          totalSupply: vault.totalSupply, tvl: vault.tvl,
          apy: vault.apy, status: vault.status,
        },
      });
    }
