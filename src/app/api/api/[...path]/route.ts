...
function handleMock(fullPath: string, req: Request, method = "GET") {
  ...
  if (p.match(/^campaigns\/[^/]+\/join$/) && method === "POST")
    return json({ success: true, data: { ok: true } });
  if (p.match(/^campaigns\/[^/]+\/claim$/) && method === "POST")
    return json({ success: true, data: { claimed: true, points: 150 } });
  ...
  if (p.match(/^tasks\/[^/]+\/verify$/) && method === "POST")
    return json({ success: true, data: { success: true, message: "Verified!" } });
  if (p.match(/^tasks\/[^/]+\/claim$/) && method === "POST")
    return json({ success: true, data: { claimed: true, points: 25 } });
  if (p.match(/^tasks\/[^/]+\/record-visit$/) && method === "POST")
    return json({ success: true, data: { ok: true } });
  ...
  if (p.match(/^users\/[^/]+\/points-history$/) && method === "GET")
    return json(MOCK_POINTS_HISTORY);
  if (p.match(/^users\/[^/]+\/profile$/) && method === "PUT")
    return json({ success: true, data: { ok: true } });
  ...
  // Leaderboard
  if ((p === "analytics/global-leaderboard" || p === "leaderboard/global") && method === "GET")
    return json(MOCK_LEADERBOARD);
  if ((p === "analytics/streak-leaderboard" || p === "leaderboard/streak") && method === "GET")
    return json(MOCK_STREAK_LEADERBOARD);
  ...
  if (p === "social-accounts" && method === "POST")
    return json({ success: true, data: { ok: true } });
  if (p === "social-accounts/unlink" && method === "POST")
    return json({ success: true, data: { ok: true } });
  ...
  if (p === "auth/refresh" && method === "POST")
    return json({ success: true, data: { accessToken: "mock-jwt", refreshToken: "mock-refresh" } });
...
const RAW_CAMPAIGNS = [
  {
    ...
    description: "Support the Tasmil ecosystem.",
    descriptionDetail: "Join the grand launch event.",
    logoUrl:
    ...
    tasks: [
      ...
      },
      {
        ...
        description: "Verify one transaction on Stellar.",
        pointReward: 75,
      },
    ],
  },
  {
    ...
    description: "Explore the hottest DeFi protocols.",
    logoUrl:
  ...
  },
  {
    ...
    description: "Mint, trade, and collect exclusive NFTs.",
    logoUrl:
  ...
  },
  {
    ...
    description: "Grow your social presence and earn rewards.",
    logoUrl:
  ...
  },
  {
    ...
    description: "Be among the first to explore next-gen Stellar.",
    logoUrl:
  ...
  },
  {
    ...
    description: "Competitive liquidity provision.",
    logoUrl:
      "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&q=80&w=1600&h=900",
    questersCount: 4800,
    rewardPoints: 250,
...
const MOCK_MY_CAMPAIGNS_PENDING = {
  success: true,
  data: {
    items: [
      {
...
const MOCK_MY_CAMPAIGNS_CLAIMABLE = {
  success: true,
  data: {
    items: [
      {
...
const MOCK_MY_CAMPAIGNS_CLAIMED = {
  success: true,
  data: {
    items: [
      {
...
const MOCK_CURRENT_SEASON = {
  data: {
    ...
    prizePoolUsdc: "5000",
    rankRewards: [
      { rankFrom: 1, rankTo: 1, usdc: "2500", points: 5000 },
