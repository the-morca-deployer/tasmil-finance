import { type NextRequest, NextResponse } from "next/server";
import { getQuestBackendBaseUrl } from "@/lib/runtime-urls";

const QUEST_URL = getQuestBackendBaseUrl();

function getAdminToken(request: NextRequest): string | null {
  const authHeader = request.headers.get("authorization");
  if (authHeader?.startsWith("Bearer ")) return authHeader.slice(7);
  return null;
}

export async function GET(request: NextRequest) {
  const token = getAdminToken(request);
  if (!token) return NextResponse.json({ message: "No admin token" }, { status: 401 });

  const limit = new URL(request.url).searchParams.get("limit") ?? "50";

  try {
    const response = await fetch(`${QUEST_URL}/api/leaderboard/global?limit=${limit}`);
    const data = await response.json();
    return NextResponse.json(data, { status: response.status });
  } catch {
    return NextResponse.json({ message: "Service unavailable" }, { status: 503 });
  }
}
