...
import { PHOENIX_TESTS } from "../helpers/test-prompts";
import { getLatestTrace, assertToolCalled } from "../helpers/langsmith";

test.describe("Phoenix DEX Protocol", () => {
  test.describe.configure({ mode: "serial" });

  for (const [action, config] of Object.entries(PHOENIX_TESTS)) {
    test(`phoenix.${action}: ${config.prompt}`, async ({ chatPage }) => {
      ...
      await chatPage.screenshotLastCard(`phoenix-${action}`);
    });
  }
});
