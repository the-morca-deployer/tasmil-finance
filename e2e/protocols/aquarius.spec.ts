...
test.describe("Aquarius AMM Protocol", () => {
  ...
  for (const [action, config] of Object.entries(AQUARIUS_TESTS)) {
    test(`aquarius.${action}: ${config.prompt}`, async ({ chatPage }) => {
      ...
      if (type === "card-clarify") {
        ...
        // Accept the primary card OR aqua-specific cards
        expect([config.expectedCard, "card-aqua-tx", "card-stellar-execute"]).toContain(nextType);
      ...
      await chatPage.screenshotLastCard(`aquarius-${action}`);
    });
  }
});
