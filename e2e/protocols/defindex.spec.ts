...
import { DEFINDEX_TESTS } from "../helpers/test-prompts";

test.describe("DeFindex Vaults", () => {
  test.describe.configure({ mode: "serial" });

  for (const [action, config] of Object.entries(DEFINDEX_TESTS)) {
    test(`defindex.${action}: ${config.prompt}`, async ({ chatPage }) => {
      ...
      await chatPage.screenshotLastCard(`defindex-${action}`);
    });
  }
});
