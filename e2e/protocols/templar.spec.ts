...
import { TEMPLAR_TESTS } from "../helpers/test-prompts";
import { getLatestTrace, assertToolCalled } from "../helpers/langsmith";

test.describe("Templar Protocol", () => {
  test.describe.configure({ mode: "serial" });

  for (const [action, config] of Object.entries(TEMPLAR_TESTS)) {
    test(`templar.${action}: ${config.prompt}`, async ({ chatPage }) => {
      ...
      await chatPage.screenshotLastCard(`templar-${action}`);
    });
  }
});
